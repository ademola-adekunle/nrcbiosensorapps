...
parser.read ("C:/OnlineBiosensorWindows_TOX/Configuration/biosensorconfig.ini") #location of biosensor configuration/parameters for floating sensor
parser2.read ("C:/OnlineBiosensorWindows_TOX/Configuration/biosensorconfig2.ini") #location of biosensor configuration/parameters for flow through sensor
import sys
import glob
import serial
...
class simpleapp_tk(Tkinter.Tk):
    ...
    def OnParameterClick(self): #what to do when the button is clicked, proceed to add as a command to the button above
        ...
        rload_label = Tkinter.Label(self.parameter,textvariable=self.parameter.rload,
                              anchor="w",fg="black",bg=defaultbg)
        rload_label.grid(column=3,row=2,columnspan=1,sticky='EW') #label position
        ...
        rload_value.grid(column=4,row=2,sticky='NW') #text entry location
...
#########DEFINITIONS OF COMMANDS FOR FLOATING SENSOR
    ...
    def OnSaveClick(self): #this save is in the parameter box
        ...
        with open ('C:/OnlineBiosensorWindows_TOX/Configuration/biosensorconfig.ini', 'w') as configfile:
    ...
    def OnStartStopClick(self): #when start is clicked
        ...
        if not os.path.exists('C:/OnlineBiosensorWindows_TOX/BiosensingData'):
                            os.makedirs('C:/OnlineBiosensorWindows_TOX/BiosensingData') #create folder for documents if it doesnt exist
        os.chmod("C:/OnlineBiosensorWindows_TOX/BiosensingData", 0o777) #make folder write and readable for all
        parser.set('Parameters','tolerance',str(tolerance))
        parser.set('Parameters','vmin', str(Vmin))
        parser.set('Parameters','alpha1', str(alpha1))
        parser.set('Parameters','graphspan', str(graphspan))
        ...
        parser.set('Parameters','restarter', str(restarter))
        parser.set('Parameters','delay', str(delay))
        parser.set('Parameters','ylimit', str(y2limit))
        parser.set('Parameters','ylimit', str(y3limit))
        parser.set('Parameters','vminacq', str(vminacq))
        parser.set('Parameters','vmaxacq', str(vmaxacq))
        parser.set('Parameters','steadyrepeat', str(steadyrepeat))
        parser.set('Parameters','steadyrepeatvmin', str(steadyrepeatvmin))
...
##        self.parameter2.destroy()
        ...
        with open ('C:/OnlineBiosensorWindows_TOX/Configuration/biosensorconfig2.ini', 'w') as configfile2:
...
if __name__ == "__main__": #creation of main. This is where you put your main code
    ...
    tab1 = ttk.Frame(tab_parent,width = 500,height = 700,)
    ...
    tab_parent.add(tab2, text="Plots:Floating biosensor")
    tab_parent.add(tab3, text="Plots:Flow through biosensor")
    ...
    frame2.grid(column=0, row=6, columnspan=5, sticky='W', padx=5, pady=10, ipadx=5, ipady=5)
    ...
    canvas.draw()
    canvas.get_tk_widget().grid(column=1,row=0,columnspan=4,rowspan=4, sticky='NSEW',padx=20, pady=20) #graph position
...
## PUMP THREAD
    ...
    def pump_control_2():
        ...
        while True:
            if flag_pump_2==1:
                ...
                print (counterpump_2)
                counterpump_2=0
                #d.getFeedback(u3.BitStateWrite(7, 1)) # Set FIO7 to output high to turn on pumpainV
    ...
    def floating(q):  #Biosensing algorithm
            ...
            if flag==1:
                ...
                if filesaveflag==1:
                    ...
                    if savecounter==1: #helps to save header and filename only once after initialization
                        ...
                        if not os.path.exists('C:/OnlineBiosensorWindows_TOX/BiosensingData'):
                            os.makedirs('C:/OnlineBiosensorWindows_TOX/BiosensingData') #create folder for documents if it doesnt exist
                        os.chdir('C:/OnlineBiosensorWindows_TOX/BiosensingData') #change directory to this new folder and save files here
                        ...
                        filename=filename
                        os.chmod("C:/OnlineBiosensorWindows_TOX/BiosensingData", 0777) #make folder write and readable for all
                        save_file= open(filename,"a") #for calling the filename where data has been saved
                        headers=['Date', 'Time', 'Vmax_Est', 'Area', 'OCV_time(s)','Tolerance', 'Vmin', 'alpha1','Vmin_Est','prev_discharge_time','DischargeArea','Ambient_Temperature','Vmax/Vmin','OCVwait','Vminwait','steadystaterepeatocv','steadystaterepeatVmin','pumprate','rload']
...
##                                        y4limit = max(vmintimevec) + (0.5*max(vmintimevec))
                                    ...
                                    a2.set_ylim([0,y3limit])
                                    canvas.draw()
                                    canvas.get_tk_widget().grid(column=1,row=6,columnspan=3,sticky='NSEW') #graph position
...
##
                                    ...
                                    if filesaveflag==1:
                                        ...
                                        datastr= (timeofsave +' ' + str(Vmax_Est)+' ' + str(Area_curve)+
                                                  ' ' + str(OCV_time)+' ' + str(tolerance)+' '+str(Vmin)+' '+str(alpha1)+' '+str(Vmin_Est)+' '+str(discharge_time)+' '+str(Disarea_curve)+' '+str(tempV)+' '+str(Sensoroutput)+' '+str(ocvwait)+' '+str(vminwait)+' '+str(steadyrepeat)+' '+str(steadyrepeatvmin)+str(targetpumprate)+' '+str(rload))
...
##                                        y4limit2 = max(vmintimevec2) + (0.5*max(vmintimevec2))
                                    ...
                                    if filesaveflag_2==1:
                                        ...
                                        datastr2= (timeofsave2 +' ' + str(Vmax_Est2)+' ' + str(Area_curve2)+
                                                  ' ' + str(OCV_time2)+' ' + str(tolerance2)+' '+str(Vmin2)+' '+str(alpha1_2)+' '+str(Vmin_Est2)+' '+str(discharge_time2)+' '+str(Disarea_curve2)+' '+str(tempV2)+' '+str(Sensoroutput2)+' '+str(ocvwait2)+' '+str(vminwait2)+' '+str(steadyrepeat2)+' '+str(steadyrepeatvmin2)+ ' ' + str(pumpcontrolon2)+ ' ' + str(pumpcontroloff2) + ' ' +str(rload2))
    ...
    def dataacqusition():
        ...
        while True:
            ...
            while read==1:
                ...
                #tempVtemp= float(GetTemperature(0))  #recalculate temp value as specified by the manufacturer
                try:
