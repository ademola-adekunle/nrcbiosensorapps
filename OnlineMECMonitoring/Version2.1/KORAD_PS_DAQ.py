...
class MainWindow(QMainWindow):
    ...
    #Update Labels statuslabels
    update_dataAcqSignal = pyqtSignal(bool,str)
    update_dataLogSignal = pyqtSignal(bool,str)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.update_dataAcqSignal.connect(self.updateAcquisitionLabel)
        self.update_dataLogSignal.connect(self.updateLoggingLabel)

    @QtCore.pyqtSlot(bool, str)
    def updateAcquisitionLabel(self,check, text):
        if check == False:
            self.acquisitionTimerLabel.setText(text)
        else:
            self.acquisitionTimerLabel.setText(time.strftime('%M:%S',time.gmtime(int(text))))
            
    @QtCore.pyqtSlot(bool, str)
    def updateLoggingLabel(self,check, text):
        if check == False:
            self.timerLabel.setText(text)
        else:
            self.timerLabel.setText(time.strftime('%M:%S',time.gmtime(int(text))))
    ...
    def runmonitoringStatus(self):
        tRunMonitoringStatus = threading.Thread(target = self.monitoringStatus)
        tRunMonitoringStatus.daemon = True
        tRunMonitoringStatus.start()
    
    def monitoringStatus(self):
        #print('here')
        global startUpAcq
        global startAcquisition, acqEnded, dAqON
        global startLogs, logEnded
        global endThread, threadEnded, calibStart, calibThread, phCalibrationStatus, pHStarted
        global dLogInterval, dAqInterval
        ...
        while True:
            if calibThread == True:
                self.update_dataAcqSignal.emit(False,"--:--")
                self.update_dataLogSignal.emit(False, "--:--")
                time.sleep(0.5)
            ...
            if dAqON == True and (startUpAcq == True):
            ...
            if dAqON == True:
                if acqEnded == False and not(self.acquisitionTimerLabel == "Acquiring" or self.acquisitionTimerLabel == "--:--"):
                    self.update_dataAcqSignal.emit(False,"Acquiring")
                elif acqEnded == True:
                    #self.dlog_time_left_int -=1
                    self.daq_time_left_int -= 1
                    self.update_dataAcqSignal.emit(True, str(self.daq_time_left_int))
                    
                
                if logEnded == False and not(self.timerLabel == "Logging" or self.timerLabel == "--:--"):
                    self.update_dataLogSignal.emit(False, "Logging")
                elif logEnded == True:
                    self.dlog_time_left_int -= 1
                    self.update_dataLogSignal.emit(True, str(self.dlog_time_left_int))
                    
                if self.daq_time_left_int == 0:
                    self.daq_time_left_int = int(dAqInterval * 60)# * 60 minutes -> seconds
                    startAcquisition = True
                    acqEnded = False
                    
                if self.dlog_time_left_int == 0:
                    self.dlog_time_left_int = int(dLogInterval * 60)
                    startLogs = True
                    logEnded = False
            else:
                self.daq_time_left_int = int(dAqInterval * 60)
                self.dlog_time_left_int = int(dLogInterval * 60)
    ...
    def mainThread(self):
        ...
        global dev1, dev2
        global startAcquisition, acqEnded
        global startLogs, logEnded
        global device_list
    ...
    def on_start_button_clicked(self):
        global ps, is_editing_setvals, psStatus, dAqON, runPS, psAval, polling, autoContinueorSelection, data_points_int, startUpAcq
