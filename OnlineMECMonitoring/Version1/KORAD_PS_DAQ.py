...
# Tested for one power supply as of Aug 21, 2020.
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        global is_editing_setvals, koradports, y1_label, y2_label
        super(MainWindow, self).__init__(*args, **kwargs)
        uic.loadUi("UI_Files/RPi_GUI_v1-7.ui", self)
        ...
        self.data_line1 = self.graphWidget.plot(self.x, self.y1plot, pen = self.pen1, symbolBrush = 'r', symbol = 'o', name = y1_label)
        self.data_line2 = self.graphWidget.plot(self.x, self.y2plot, pen = self.pen2, symbolBrush = 'b', symbol = 'o', name = y2_label)
    ...
    def check_plot_vars(self):
        ...
        self.currentPlot = self.currentSave[-data_points_int:]
        self.voltagePlot = self.voltageSave[-data_points_int:]

        # include temperature and pH retrieval here as well
        self.check_y_labels()

    def check_y_labels(self):
        global y1_var, y1_label, y2_var, y2_label

        if y1_var == 'Current':
            self.y1plot = self.currentPlot
            y1_label = 'Current, mA'

        elif y1_var == 'Voltage':
            self.y1plot = self.voltagePlot
            y1_label = 'Voltage, V'

        if y2_var == 'Current':
            self.y2plot = self.currentPlot
            y2_label = 'Current, mA'

        elif y2_var == 'Voltage':
            self.y2plot = self.voltagePlot
            y2_label = 'Voltage, V'
    ...
    def on_start_button_clicked(self):
        global ps, is_editing_setvals, psStatus, dAqON, runPS, psAval
    ...
    def update_plot(self, date):
        ...
        self.x.append(time.strftime("%H:%M:%S", time.gmtime(date)))

        x = self.x[-data_points_int:] # takes last n number of data points from array

        self.xplot = dict(enumerate(x))

        if len(self.xplot) <= 6:
            self.stringaxis.setTicks([list(self.xplot.items())])
        else:
            self.stringaxis.setTicks([list(self.xplot.items())[::int(len(self.xplot)/6)], list(self.xplot.items())[1::int(len(self.xplot)/6)]])

        self.xplot = list(self.xplot.keys())

        self.data_line1.setData(self.xplot, self.y1plot, name = y1_label)
        self.data_line2.setData(self.xplot, self.y2plot, name = y2_label)

    def update_statusbar(self):
        global psStatus
        """
        if str(psStatus) == 'ChannelMode.constant_voltage':
            ps_const_status = 'Constant Voltage'
        elif str(psStatus) == 'ChannelMode.constant_current':
            ps_const_status = 'Constant Current'
        else:
            ps_const_status = 'TROUBLE!'
        """
...
try:
    ...
    if psAval == True:
        ...
        if runPS == False:
            main.ps_output_status = 'off'
            main.update_statusbar()

        else:
            main.startButton.animateClicked()
